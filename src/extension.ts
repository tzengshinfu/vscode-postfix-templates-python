...
export async function activate(context: vsc.ExtensionContext): Promise<void> {
    ...
    try {
        const wasm1 = vsc.Uri.joinPath(context.extensionUri, 'out', 'tree-sitter-python.wasm').fsPath
        const wasm2 = vsc.Uri.joinPath(context.extensionUri, 'out', 'out', 'tree-sitter-python.wasm').fsPath
        const wasmPath = existsSync(wasm1) ? wasm1 : wasm2
