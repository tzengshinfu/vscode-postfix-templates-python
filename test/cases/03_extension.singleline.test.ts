...
describe('03. Single line template tests', () => {
  ...
  describe('custom template with snippet variables', () => {
    const run = runWithCustomTemplate('print($TM_LINE_NUMBER, {{expr}})')

    run('identifier', 'expr{custom}  |  expr{custom}  >>  print(1, expr)')
  })

  describe('custom template with escaped variable syntax', () => {
    const run = runWithCustomTemplate('print("\\$TM_LINE_NUMBER", \\$1.{{expr}})')

    run('identifier', 'expr{custom}  |  expr{custom}  >>  print("$TM_LINE_NUMBER", $1.expr)')
  })
